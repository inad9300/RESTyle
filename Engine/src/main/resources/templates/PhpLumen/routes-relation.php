...
{{#unless isBelongTo}}$app->post('/{{resourceRoute}}/{id}/{{subresourceRoute}}', ['middleware' => [{{#unless guestCanCreateSubresource}}'auth', {{/unless}}], 'uses' => '{{resourceClass}}Controller@create{{subresourceClassPlural}}']);{{/unless}}
{{#unless isBelongTo}}{{#if isManyToMany}}
